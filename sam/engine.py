...
def prompt_sam_with_bboxes(sam_predictor, data, box_result):
    ...
    target_size = sam_predictor.transform.get_preprocess_shape(
        img.shape[2], img.shape[3], sam_predictor.transform.target_length
    )
    try:
        ...
        transformed_img = F.interpolate(
            img, target_size, mode="bilinear", align_corners=False, antialias=True
        )
    except TypeError:
        transformed_img = F.interpolate(
    ...
    # extract img feature
    sam_predictor.features = sam_predictor.model.image_encoder(transformed_img).to(
        sam_predictor.model.device
    )
...
def single_gpu_test(
    model, sam_predictor, data_loader, show=False, out_dir=None, show_score_thr=0.3
):
    model.eval()
    results = []
