...
def parse_args():
    ...
    parser.add_argument(
        "--test-img", default=None, type=str, nargs="+", help="image used to test model"
    )
    parser.add_argument(
        "--work-dir", default=os.getcwd(), help="the dir to save logs and models"
    )
    parser.add_argument(
        "--calib-dataset-cfg",
        help=(
            "dataset config path used to calibrate in int8 mode. If not "
    ...
    parser.add_argument("--show", action="store_true", help="Show detection outputs")
    parser.add_argument(
        "--dump-info", action="store_true", help="Output information for SDK"
    )
    parser.add_argument(
        "--quant-image-dir", default=None, help="Image directory for quantize model."
    )
    parser.add_argument(
    ...
    if backend == Backend.RKNN:
        ...
        if get_rknn_quantization(deploy_cfg) is True:
            ...
            common_params.update(
                dict(mean_values=[transform["mean"]], std_values=[transform["std"]])
            )
