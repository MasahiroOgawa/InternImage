...
def build_loader(config):
    ...
    config.freeze()
    print(
        f"local rank {config.LOCAL_RANK} / global rank {dist.get_rank()}"
...
def build_dataset(split, config):
    ...
    if config.DATA.DATASET == "imagenet":
        ...
        raise NotImplementedError(f"build_dataset does support {config.DATA.DATASET}")
