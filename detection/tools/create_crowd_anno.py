...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate MMDetection Annotations for Crowdhuman-like dataset"
    )
    parser.add_argument("--dataset", help="dataset name", type=str)
    parser.add_argument(
        "--dataset-split", help="dataset split, e.g. train, val", type=str
    )
...
def decode_annotations(records, dataset_path):
    ...
    for idx, rec_id in enumerate(rec_ids):
        ...
        img_dict = dict(file_name=img_id + ".jpg", height=im_h, width=im_w, id=idx)
    ...
    json_dict = dict(images=img_list, annotations=ann_list, categories=cate_list)
    return json_dict
