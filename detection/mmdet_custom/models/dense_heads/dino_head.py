...
class DINOHead(DeformableDETRHead):
    ...
    def forward_train(
        ...
        **kwargs,
        ...
        dn_label_query, dn_bbox_query, attn_mask, dn_meta = self.dn_generator(
            gt_bboxes, gt_labels, self.label_embedding, img_metas
        )
    ...
    def forward(
        ...
        query_embeds = None
        hs, inter_references, topk_score, topk_anchor = self.transformer(
            mlvl_feats,
            mlvl_masks,
            query_embeds,
            mlvl_positional_encodings,
    ...
    def loss_dn_single(
        ...
        cls_reg_targets = self.get_dn_target(
            bbox_preds_list, gt_bboxes_list, gt_labels_list, img_metas, dn_meta
        )
        (
            labels_list,
            label_weights_list,
    ...
    def _get_dn_target_single(
        self, dn_bbox_pred, gt_bboxes, gt_labels, img_meta, dn_meta
    ):
        ...
        if len(gt_labels) > 0:
            ...
            pos_inds = (
                torch.tensor(range(num_groups)) * single_pad
