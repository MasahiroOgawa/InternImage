...
class DnQueryGenerator(BaseModule):
    ...
    def forward(self, gt_bboxes, gt_labels=None, label_enc=None, img_metas=None):
        """
        Args:
        ...
        if self.training:
            ...
            positive_idx += (
                (torch.tensor(range(num_groups)) * len(boxes) * 2)
                .long()
                .cuda()
                .unsqueeze(1)
            )
            ...
            if self.box_noise_scale > 0:
                ...
                rand_part *= rand_sign
                known_bbox_ += torch.mul(rand_part, diff).cuda() * self.box_noise_scale
