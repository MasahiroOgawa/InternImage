        ...
        if idx == 0:  # front view image
            if gt_te is not None:
                for bbox, attr in zip(gt_te, gt_te_attr):
                    ...
                    image = render_corner_rectangle(
                        image, (b[0], b[1]), (b[2], b[3]), COLOR_DICT[attr], 3, 1
                    )
            ...
            if pred_te is not None:
                for bbox, attr in zip(pred_te, pred_te_attr):
                    b = bbox.astype(np.int32)
                    image = cv2.rectangle(
                        image, (b[0], b[1]), (b[2], b[3]), COLOR_DICT[attr], 3
                    )
...
def render_corner_rectangle(
    ...
    # edges
    img = cv2.line(
        img, (pt1[0] + corner_length, pt1[1]), (pt2[0] - corner_length, pt1[1]), *e_args
    )
    img = cv2.line(
        img, (pt2[0], pt1[1] + corner_length), (pt2[0], pt2[1] - corner_length), *e_args
    )
    img = cv2.line(
        img, (pt1[0], pt1[1] + corner_length), (pt1[0], pt2[1] - corner_length), *e_args
    )
    img = cv2.line(
        img, (pt1[0] + corner_length, pt2[1]), (pt2[0] - corner_length, pt2[1]), *e_args
    )
...
def render_bev(
    ...
    pred_topology_lclc=None,
    map_size=[-52, 52, -27, 27],
    scale=20,
):
    image = np.zeros(
        (
            int(scale * (map_size[1] - map_size[0])),
            int(scale * (map_size[3] - map_size[2])),
            3,
        ),
        dtype=np.uint8,
    )
    ...
    if gt_lc is not None:
        for lc in gt_lc:
            draw_coor = (
                scale * (-lc[:, :2] + np.array([map_size[1], map_size[3]]))
            ).astype(np.int)
            image = cv2.polylines(
                image,
                [draw_coor[:, [1, 0]]],
            ...
            image = cv2.circle(
                image,
                (draw_coor[0, 1], draw_coor[0, 0]),
                max(round(scale * 0.5), 3),
                COLOR_GT,
                -1,
            )
            image = cv2.circle(
                image,
                (draw_coor[-1, 1], draw_coor[-1, 0]),
                max(round(scale * 0.5), 3),
                COLOR_GT,
                -1,
            )
        ...
        for l1_idx, lclc in enumerate(gt_topology_lclc):
            for l2_idx, connected in enumerate(lclc):
                if connected:
                    ...
                    l2_mid = len(l2) // 2
                    p1 = (
                        scale * (-l1[l1_mid, :2] + np.array([map_size[1], map_size[3]]))
                    ).astype(np.int)
                    p2 = (
                        scale * (-l2[l2_mid, :2] + np.array([map_size[1], map_size[3]]))
                    ).astype(np.int)
                    image = cv2.arrowedLine(
                        image,
                        (p1[1], p1[0]),
...
class OpenLaneV2SubsetADataset(Custom3DDataset):
    ...
    def get_data_info(self, index):
        ...
        for i, camera in enumerate(frame.get_camera_list()):
            assert (
                camera == "ring_front_center" if i == 0 else True
    ...
    def get_ann_info(self, index):
        ...
        gt_lc = np.array(
            [lc["points"] for lc in frame.get_annotations_lane_centerlines()],
            dtype=np.float32,
        )
        ...
        gt_te = np.array(
            [
                element["points"].flatten()
                for element in frame.get_annotations_traffic_elements()
            ],
            dtype=np.float32,
        ).reshape(-1, 4)
        gt_te_labels = np.array(
            [
                element["attribute"]
                for element in frame.get_annotations_traffic_elements()
            ],
        ...
        assert (
            gt_lc.shape[0]
            == gt_topology_lclc.shape[0]
            == gt_topology_lclc.shape[1]
            == gt_topology_lcte.shape[0]
        )
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        metric_results = {}
        for key, val in openlanev2_evaluate(
            ground_truth=self.ann_file, predictions=pred_dict
        ).items():
    ...
    def format_preds(self, results):
        ...
        for index, result in enumerate(results):
            ...
            lanes, confidences = (
                pred_lc[0][sorted_index],
                pred_lc[1][:, 0][sorted_index],
            )
    ...
    def visualize(
        ...
        **kwargs,
        ...
        assert (
            visualization_dir
        ), "Please specify visualization_dir for saving visualization."
        ...
        for index, (key, prediction) in enumerate(pred_dict["results"].items()):
            ...
            try:
                metric_results = openlanev2_evaluate(
                    gt_result, pred_result, verbose=False
                )
            except Exception:
                metric_results = None
            ...
            # filter lc
            pred_lc_mask = (
                np.array([lc["confidence"] for lc in prediction["lane_centerline"]])
                > confidence_threshold
            )
            pred_lc = np.array([lc["points"] for lc in prediction["lane_centerline"]])[
                pred_lc_mask
            ]

            # filter te
            pred_te_mask = (
                np.array([te["confidence"] for te in prediction["traffic_element"]])
                > confidence_threshold
            )
            pred_te = np.array(
                [te["points"].flatten() for te in prediction["traffic_element"]]
            )[pred_te_mask]
            pred_te_attr = np.array(
                [te["attribute"] for te in prediction["traffic_element"]]
            )[pred_te_mask]

            # filter topology
            pred_topology_lclc = (
                prediction["topology_lclc"][pred_lc_mask][:, pred_lc_mask]
                > confidence_threshold
            )
            pred_topology_lcte = (
                prediction["topology_lcte"][pred_lc_mask][:, pred_te_mask]
                > confidence_threshold
            )
            ...
            if frame.get_annotations():
                ...
                gt_lc, gt_te, gt_te_attr, gt_topology_lclc, gt_topology_lcte = (
                    None,
                    None,
                    None,
                    None,
            ...
            images = render_pv(
                ...
                pred_lc=pred_lc,
                gt_te=gt_te,
                gt_te_attr=gt_te_attr,
                pred_te=pred_te,
                pred_te_attr=pred_te_attr,
            )
            ...
            output_path = os.path.join(
                visualization_dir, f'{"/".join(key)}/pv_multiview.jpg'
            )
            mmcv.imwrite(multiview, output_path)

            front_view = render_front_view(
                images[0],
                data_info["lidar2img"][0],
                gt_lc=gt_lc,
                pred_lc=pred_lc,
                gt_te=gt_te,
                pred_te=pred_te,
            ...
            bev_lane = render_bev(
                gt_lc=gt_lc,
                pred_lc=pred_lc,
                map_size=[-52, 55, -27, 27],
                scale=20,
            )
            bev_lane = cv2.putText(
                bev_lane, info, (30, 45), cv2.FONT_HERSHEY_SIMPLEX, 0.8, COLOR_GT, 2
            )
            output_path = os.path.join(
                visualization_dir, f'{"/".join(key)}/bev_lane.jpg'
            )
            ...
            bev_topology = cv2.putText(
                bev_topology, info, (30, 45), cv2.FONT_HERSHEY_SIMPLEX, 0.8, COLOR_GT, 2
            )
            output_path = os.path.join(
                visualization_dir, f'{"/".join(key)}/bev_topology.jpg'
            )
