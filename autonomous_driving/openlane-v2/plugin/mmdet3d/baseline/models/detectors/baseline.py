...
class Baseline(MVXTwoStageDetector):
    ...
    def simple_forward(self, img, img_metas):
                ...
                [
                    torch.cat(
                        [
                            ...
                            for l in img_metas[b]["lidar2img"]
                        ],
                        dim=0,
                    ).unsqueeze(0)
                    for b in range(B)
        ...
        te_img_metas = [
            {
                "batch_input_shape": (
                    img_metas[b]["pad_shape"][0][0],
                    img_metas[b]["pad_shape"][0][1],
                ),
                "img_shape": (
                    img_metas[b]["img_shape"][0][0],
                    img_metas[b]["img_shape"][0][1],
                    None,
                ),
    ...
    def forward_train(
        ...
        **kwargs,
        ...
        losses.update({f"lc_{key}": val for key, val in lc_loss_dict.items()})
        ...
        losses.update({f"te_{key}": val for key, val in te_loss_dict.items()})
        ...
        losses.update(
            {
                f"topology_lclc_{key}": val
                for key, val in topology_lclc_loss_dict.items()
            }
        )
        ...
        losses.update(
            {
                f"topology_lcte_{key}": val
                for key, val in topology_lcte_loss_dict.items()
            }
        )
    ...
    def forward_test(self, img, img_metas, **kwargs):
        ...
        assert len(pred_lc) == len(pred_te) == 1, "evaluation implemented for bs=1"
        return [
            {
                "pred_lc": pred_lc[0],
                "pred_te": pred_te[0],
