...
class ROAD_BEVFormer(MVXTwoStageDetector):
    def __init__(
        ...
        super(ROAD_BEVFormer, self).__init__(
            pts_voxel_layer,
            pts_voxel_encoder,
            pts_middle_encoder,
            pts_fusion_layer,
    ...
    def forward_train(
        ...
        lane_losses, lane_assign_result = self.pts_bbox_head.loss(
            *loss_inputs, img_metas=img_metas
        )
        ...
        if self.bbox_head is not None:
            ...
            bbox_losses, te_assign_result = self.bbox_head.loss(
                bbox_outs, gt_te, gt_te_labels, bbox_img_metas, gt_bboxes_ignore
            )
            ...
            if self.lcte_head is not None:
                ...
                lcte_losses = self.lcte_head.forward_train(
                    lane_feats,
                    lane_assign_result,
                    te_feats,
                    te_assign_result,
    ...
    def simple_test_pts(self, x, img_metas, img=None, prev_bev=None, rescale=False):
        ...
        lane_results = self.pts_bbox_head.get_lanes(outs, img_metas, rescale=rescale)
        ...
        if self.bbox_head is not None:
            ...
            bbox_results = self.bbox_head.get_bboxes(
                bbox_outs, bbox_img_metas, rescale=rescale
            )
        else:
    ...
    def simple_test(self, img_metas, img=None, prev_bev=None, rescale=False):
        ...
        new_prev_bev, bbox_results, lane_results, lclc_results, lcte_results = (
            self.simple_test_pts(img_feats, img_metas, img, prev_bev, rescale=rescale)
        )
        for result_dict, bbox, lane, lclc, lcte in zip(
            results_list, bbox_results, lane_results, lclc_results, lcte_results
        ):
