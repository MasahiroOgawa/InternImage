...
class CustomDETRHead(DETRHead):
    ...
    def _get_target_single(
        ...
        if self.object_type == "lane":
            ...
            for p in range(self.num_reg_dim // 3):
                pos_gt_bboxes_normalized[..., 3 * p] = (
                    pos_gt_bboxes[..., 3 * p] - self.bev_range[0]
                ) / (self.bev_range[3] - self.bev_range[0])
                pos_gt_bboxes_normalized[..., 3 * p + 1] = (
                    pos_gt_bboxes[..., 3 * p + 1] - self.bev_range[1]
                ) / (self.bev_range[4] - self.bev_range[1])
                pos_gt_bboxes_normalized[..., 3 * p + 2] = (
                    pos_gt_bboxes[..., 3 * p + 2] - self.bev_range[2]
    ...
    def _get_bboxes_single(
        ...
        if self.object_type == "lane":
            ...
            for p in range(self.num_reg_dim // 3):
                det_bboxes[..., 3 * p] = (
                    det_bboxes[..., 3 * p] * (self.bev_range[3] - self.bev_range[0])
                    + self.bev_range[0]
                )
                det_bboxes[..., 3 * p + 1] = (
                    det_bboxes[..., 3 * p + 1] * (self.bev_range[4] - self.bev_range[1])
                    + self.bev_range[1]
                )
                det_bboxes[..., 3 * p + 2] = (
                    det_bboxes[..., 3 * p + 2] * (self.bev_range[5] - self.bev_range[2])
                    + self.bev_range[2]
                )
                det_bboxes[..., 3 * p].clamp_(
                    min=self.bev_range[0], max=self.bev_range[3]
                )
                det_bboxes[..., 3 * p + 1].clamp_(
                    min=self.bev_range[1], max=self.bev_range[4]
                )
                det_bboxes[..., 3 * p + 2].clamp_(
                    min=self.bev_range[2], max=self.bev_range[5]
                )
        else:
            # exclude background
    ...
    def onnx_export(self, **kwargs):
        raise NotImplementedError(
            f"TODO: replace 4 with self.num_reg_dim : {self.num_reg_dim}"
        )
