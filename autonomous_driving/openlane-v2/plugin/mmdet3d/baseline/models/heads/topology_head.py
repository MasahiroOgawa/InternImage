...
class TopologyHead(BaseModule):
    def __init__(
        self, in_channels, hidden_channels, out_channels, num_layers, loss_cls
    ):
        super().__init__()
    ...
    def forward(self, all_a_preds_list, all_b_preds_list):
        ...
        assert (
            self.in_channels == all_a_preds_list.shape[-1] + all_b_preds_list.shape[-1]
    ...
    def loss(self, pred_adj_list, row_assign_results, column_assign_results, gt_adj):
        ...
        for b in range(pred_adj.shape[0]):
            ...
            rs = (
                row_assign_result["pos_inds"][b]
                .unsqueeze(-1)
                .repeat(1, column_assign_result["pos_inds"][b].shape[0])
            )
            cs = (
                column_assign_result["pos_inds"][b]
                .unsqueeze(0)
                .repeat(row_assign_result["pos_inds"][b].shape[0], 1)
            )
            target[rs, cs] = gt_adj[b][row_assign_result["pos_assigned_gt_inds"][b]][
                :, column_assign_result["pos_assigned_gt_inds"][b]
            ].float()
