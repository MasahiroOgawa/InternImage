...
def _draw_topology(image, topology, intrinsic, extrinsic):
    ...
    mid = (
        (coord_to[0] + coord_from[0]) / 2,
        (coord_to[1] + coord_from[1]) / 2 - 50,
    )
...
def draw_annotation_pv(
    camera, image, annotation, intrinsic, extrinsic, with_attribute, with_topology
):
    for lane_centerline in annotation["lane_centerline"]:
        _draw_lane_centerline(
