...
def collect(
    root_path: str, data_dict: dict, collection: str, point_interval: int = 1
) -> None:
    r"""
    ...
    meta = {
        (split, segment_id, timestamp): io.json_load(
            f"{root_path}/{split}/{segment_id}/info/{timestamp}.json"
        )
        for split, segment_id, timestamp in tqdm(
            data_list, desc=f"collecting {collection}", ncols=80
        )
    }
    ...
    for identifier, frame in meta.items():
        ...
        for camera in meta[identifier]["sensor"].keys():
            for para in ["intrinsic", "extrinsic"]:
                for k, v in meta[identifier]["sensor"][camera][para].items():
                    meta[identifier]["sensor"][camera][para][k] = np.array(
                        v, dtype=np.float64
                    )
        ...
        for i, traffic_element in enumerate(frame["annotation"]["traffic_element"]):
            meta[identifier]["annotation"]["traffic_element"][i]["points"] = np.array(
                traffic_element["points"], dtype=np.float32
            )
        meta[identifier]["annotation"]["topology_lclc"] = np.array(
            meta[identifier]["annotation"]["topology_lclc"], dtype=np.int8
        )
        meta[identifier]["annotation"]["topology_lcte"] = np.array(
            meta[identifier]["annotation"]["topology_lcte"], dtype=np.int8
        )
