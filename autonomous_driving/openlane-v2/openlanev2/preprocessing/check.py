...
def check_results(results: dict) -> None:
        ...
        if key in results:
            ...
            if key == "country / region":
                try:
                    ...
                    raise Exception(
                        f"Please specify a valid [{key}] according to ISO3166"
                    )
        else:
            valid = False
    ...
    for token, predictions in results["results"].items():
        ...
        if not isinstance(predictions, dict):
            raise Exception(
                f"Type of value in key [results/{token}/predictions] should be dict"
            )
        ...
        for key in ["lane_centerline", "traffic_element"]:
            ...
            if not isinstance(predictions[key], list):
                raise Exception(
                    f"Type of value in key [results/{token}/predictions/{key}] should be list"
                )
            ...
            for instance in predictions[key]:
                for k in ["id", "points", "confidence"]:
                    if k not in instance:
                        raise Exception(
                            f"Miss key [results/{token}/predictions/{key}/k]."
                        )
                ...
                if key == "lane_centerline" and not (
                ...
                if key == "traffic_element" and not (
                    points.ndim == 2 and points.shape == (2, 2)
                ):
                    raise Exception(
        ...
        if not isinstance(topology_lclc, np.ndarray):
            raise Exception(
                f"Type of value in key [results/{token}/predictions/topology_lclc] should be np.ndarray"
            )
        topology_lclc = np.array(topology_lclc)
        if not (
            topology_lclc.ndim == 2
            and topology_lclc.shape[0] == len(ids["lane_centerline"])
            and topology_lclc.shape[1] == len(ids["lane_centerline"])
        ):
            raise Exception(
        ...
        if not isinstance(topology_lcte, np.ndarray):
            raise Exception(
                f"Type of value in key [results/{token}/predictions/topology_lcte] should be np.ndarray"
            )
        topology_lcte = np.array(topology_lcte)
        if not (
            topology_lcte.ndim == 2
            and topology_lcte.shape[0] == len(ids["lane_centerline"])
            and topology_lcte.shape[1] == len(ids["traffic_element"])
        ):
            raise Exception(
