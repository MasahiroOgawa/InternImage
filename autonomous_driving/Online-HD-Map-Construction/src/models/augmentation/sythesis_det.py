...
class NoiseSythesis(nn.Module):
    ...
    def gaussian_noise_bbox(self, bbox):
        ...
        if self.quantify:
            ...
            for i in range(self.bbox_coord_dim):
                noisy_bbox[..., i] = torch.clamp(
                    noisy_bbox[..., 0], 1, self.canvas_size[i]
                )
        else:
    ...
    def gaussian_noise_poly(self, polyline, polyline_mask):
        ...
        if self.quantify:
            ...
            for i in range(self.poly_coord_dim):
                noisy_polyline[..., i] = torch.clamp(
                    noisy_polyline[..., i], 0, self.canvas_size[i]
                )
    ...
    def random_apply(self, bbox):
        ...
        # prevent out of bound case
        bbox[..., 0] = torch.clamp(bbox[..., 0], 0, self.canvas_size[0])

        bbox[..., 1] = torch.clamp(bbox[..., 1], 0, self.canvas_size[1])
