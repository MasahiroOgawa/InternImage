...
class UpsampleBlock(nn.Module):
    def __init__(self, ins, outs):
        ...
        self.conv = nn.Conv2d(ins, outs, kernel_size=3, stride=1, padding=1)  # same
...
class IPMEncoder(nn.Module):
    ...
    def __init__(
        ...
        if self.use_lidar:
            ...
            self.outconvs = nn.Conv2d(
                (self.upsample.out_channels + 3) * len(heights),
                out_channels // 2,
                kernel_size=3,
                stride=1,
                padding=1,
            ...
            self.outconvs_lidar = nn.Conv2d(
                lidar_dim, _out_channels, kernel_size=3, stride=1, padding=1
            )  # same
        else:
            self.outconvs = nn.Conv2d(
                (self.upsample.out_channels + 3) * len(heights),
    ...
    def ipm(self, cam_feat, ego2cam, img_shape):
        ...
        projected_feature = F.grid_sample(
            cam_feat, bev_grid_pos, align_corners=False
        ).view(self.B, -1, C, nlvl, bH, bW)  # B,cam,C,nlvl,bH,bW
        ...
        bev_grid = bev_grid.view(self.B, nlvl, bH, bW, 3).permute(0, 4, 1, 2, 3)
