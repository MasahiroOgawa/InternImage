...
from .detgen_utils.causal_trans import (
    CausalTransformerDecoder,
    CausalTransformerDecoderLayer,
)
from .detgen_utils.utils import (
    dequantize_verts,
...
class PolylineGenerator(nn.Module):
    ...
    def __init__(
        ...
        self.input_proj = nn.Conv2d(in_channels, self.embedding_dim, kernel_size=1)

        decoder_layer = CausalTransformerDecoderLayer(
            **decoder_config.pop("layer_config")
        )
        self.decoder = CausalTransformerDecoder(decoder_layer, **decoder_config)
    ...
    def _init_embedding(self):
        if self.class_conditional:
            self.label_embed = nn.Embedding(self.num_classes, self.embedding_dim)

        self.coord_embed = nn.Embedding(self.coord_dim, self.embedding_dim)
        self.pos_embeddings = nn.Embedding(self.max_seq_length, self.embedding_dim)

        # to indicate the role of the position is the start of the line or the end of it.
        self.bbox_context_embed = nn.Embedding(
            self.condition_points_num, self.embedding_dim
        )
    ...
    def _prepare_context(self, batch, context):
        ...
        image_embeddings = assign_bev(context["bev_embeddings"], batch["lines_bs_idx"])
        ...
        image_coords = torch.stack(torch.meshgrid(Hs, Ws), dim=-1).to(device)
    ...
    def _embed_inputs(self, seqs, condition_embedding=None):
        ...
        pos_embeddings = self.pos_embeddings(
            (torch.arange(seq_len, device=seqs.device) / self.coord_dim).floor().long()
        )  # seq_len, h
    ...
    def sperate_forward(self, batch, context, **kwargs):
        ...
        for i, (_poly_logits, size) in enumerate(zip(polyline_logits, sizes)):
            if size < sizes[1]:
                _poly_logits = F.pad(
                    _poly_logits, (0, 0, 0, sizes[1] - size), "constant", 0
                )
    ...
    def _forward_train(self, batch: dict, context: dict, **kwargs):
        ...
        """
        # we use the gt vertices
        global_context, seq_context = self._prepare_context(batch, context)
    ...
    @force_fp32(
        apply_to=("global_context_embedding", "sequential_context_embeddings", "cache")
    )
    def body(
        self,
        seqs,
        ...
        decoder_inputs = self._embed_inputs(seqs, global_context_embedding)
        ...
        logits[:, 1 :: self.coord_dim] = (
            logits[:, 1 :: self.coord_dim] * vertices_mask_y - ~vertices_mask_y * 1e9
        )
        ...
        if self.coord_dim > 2:
            ...
            logits[:, 2 :: self.coord_dim] = (
                logits[:, 2 :: self.coord_dim] * vertices_mask_z
                - ~vertices_mask_z * 1e9
            )
    ...
    def loss(self, gt: dict, pred: dict):
        ...
        loss = (
            -torch.sum(pred["polylines"].log_prob(gt["polylines"]) * mask * weight)
            / weight.sum()
        )
    ...
    def inference(
        ...
        **kwargs,
        ...
        samples = torch.empty([batch_size, 0], dtype=torch.int32, device=device)
        ...
        outputs = self.post_process(
            decoded_tokens, seq_len, device, only_return_complete
        )

        return outputs

    def post_process(
        self, polyline, max_seq_len=None, device=None, only_return_complete=True
    ):
        """
