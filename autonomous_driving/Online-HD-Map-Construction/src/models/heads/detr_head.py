...
class DETRMapFixedNumHead(BaseMapHead):
        ...
        **kwargs,
        ...
        if patch_size is not None:
            self.register_buffer(
                "patch_size",
                torch.tensor((patch_size[1], patch_size[0])),
            )
    ...
    def forward(self, bev_feature, img_metas=None):
        ...
        outs_dec, _ = self.transformer(
            x, masks.type(torch.bool), self.query_embedding.weight, pos_embed
        )
    ...
    def _get_target_single(
        self, score_pred, lines_pred, gt_lines, gt_labels, gt_bboxes_ignore=None
    ):
        """
        ...
        return (labels, label_weights, lines_target, lines_weights, pos_inds, neg_inds)
    ...
    def loss_single(
        ...
        new_gts, num_total_pos, num_total_neg, pos_inds_list = self.get_targets(
            preds, gts, gt_bboxes_ignore_list
        )
        ...
        if reduction == "none":  # For performance analysis
            loss_reg = self.reg_loss(
                lines_preds,
                new_gts["lines_targets"],
                new_gts["lines_weights"],
