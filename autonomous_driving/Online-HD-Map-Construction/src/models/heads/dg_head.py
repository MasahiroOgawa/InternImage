...
class DGHead(BaseMapHead):
    ...
    def forward_train(self, batch: dict, context: dict, only_det=False, **kwargs):
        ...
        losses_dict, det_match_idxs, det_match_gt_idxs = self.loss_det(batch, outs)

        if only_det:
            return outs, losses_dict

        if self.augmentation is not None:
            polylines, bbox_flat = self.augmentation(batch["gen"], simple_aug=True)
        ...
        if self.joint_training:
            ...
            # quantize the data
            _bboxs = torch.round(_bboxs).type(torch.int32)
            ...
            for k in gen_input.keys():
                if k == "bbox_flat":
                    ...
                    gen_input[k] = torch.cat(
                        (gen_input[k], gen_input[k][remain_idx]), dim=0
                    )
        ...
        if self.joint_training:
            for k in batch["gen"].keys():
                batch["gen"][k] = torch.cat(
                    (batch["gen"][k], batch["gen"][k][remain_idx]), dim=0
                )

        gen_losses_dict = self.loss_gen(batch, outs)
    ...
    def loss_det(self, gt: dict, pred: dict):
        ...
        # det
        det_loss_dict, det_match_idx, det_match_gt_idx = self.det_net.loss(
            gt["det"], pred["bbox"]
        )
    ...
    @torch.no_grad()
    def inference(
        self, batch: dict = {}, context: dict = {}, gt_condition=False, **kwargs
    ):
        """
    ...
    def post_process(self, preds: dict, tokens, gts: dict = None, **kwargs):
        ...
        for batch_idx in range(len(tokens)):
            ...
            # for gen results.
            batch2seq = np.nonzero(preds["lines_bs_idx"].cpu().numpy() == batch_idx)[0]

            ret_dict_single.update({"nline": len(batch2seq), "vectors": []})
...
def pack_groundtruth(
    batch_idx, gts, tokens, range_size, gen_net_name="gen", coord_dim=2
):
    if "keypoints" in gts["det"]:
        gt_bbox = gts["det"]["keypoints"][batch_idx].detach().cpu().numpy()
    else:
        gt_bbox = gts["det"]["bbox"][batch_idx].detach().cpu().numpy()
    det_gt = {
        "labels": gts["det"]["class_label"][batch_idx].detach().cpu().numpy(),
        "bboxes": gt_bbox,
    }

    batch2seq = np.nonzero(gts["gen"]["lines_bs_idx"].cpu().numpy() == batch_idx)[0]
    ...
    for i in batch2seq:
        gt_line = gts["gen"]["polylines"].detach().cpu().numpy()[i]
