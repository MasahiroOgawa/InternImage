...
class HungarianLinesAssigner(BaseAssigner):
    ...
    def assign(self, preds: dict, gts: dict, gt_bboxes_ignore=None, eps=1e-7):
        """
        ...
        assigned_gt_inds = preds["lines"].new_full((num_lines,), -1, dtype=torch.long)
        assigned_labels = preds["lines"].new_full((num_lines,), -1, dtype=torch.long)
        ...
        try:
            ...
            ipdb.set_trace()
        matched_row_inds = torch.from_numpy(matched_row_inds).to(preds["lines"].device)
        matched_col_inds = torch.from_numpy(matched_col_inds).to(preds["lines"].device)
