...
class PolygonizeLocalMapBbox(object):
            ...
            for polyline in _lines:
                ...
                if label == 2:
                    ...
                    polyline_weight = np.pad(
                        polyline_weight, ((0, 1),), constant_values=1.0
                    )
                    polyline_weight = polyline_weight / polyline_weight.sum()

                # flatten and quantilized
                fpolyline = quantize_verts(polyline, self.canvas_size, self.coord_dim)

                fpolyline = fpolyline.reshape(-1)

                # reindex starting from 1, and add a zero stopping token(EOS),
                fpolyline = np.pad(
                    fpolyline + self.coord_dim_start_idx, ((0, 1),), constant_values=0
                )
    ...
    def Polygonization(self, input_dict):
        ...
        polyline_map, polyline_map_mask, polyline_map_weight = self.format_polyline_map(
            vectors
        )

        keypoint, keypoint_label, qkeypoint, qkeypoint_mask = self.format_keypoint(
            vectors
        )
...
def evaluate_line(polyline):
    ...
    pts_weight = np.pad(pts_weight, ((0, 1)), constant_values=1 / (len(polyline) * 2))
...
def quantize_verts(verts, canvas_size, coord_dim):
    ...
    verts_ratio = (verts[:, :coord_dim] - min_range) / (max_range - min_range)
