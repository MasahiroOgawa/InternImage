...
class AV2Dataset(BaseMapDataset):
    ...
    def get_sample(self, idx):
        ...
        if not self.test_mode:
            ...
            for k, v in ann.items():
                if k in self.cat2id.keys():
                    map_label2geom[self.cat2id[k]] = [
                        LineString(np.array(l)[:, :3]) for l in v
                    ]

        ego2img_rts = []
        cams = sample["sensor"]
        for c in cams.values():
            extrinsic, intrinsic = np.array(c["extrinsic"]), np.array(c["intrinsic"])
        ...
        input_dict = {
            ...
            "img_filenames": [
                os.path.join(self.root_path, c["image_path"]) for c in cams.values()
            ],
