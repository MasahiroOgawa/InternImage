...
def parse_args():
    ...
    parser.add_argument("log_id", type=str, help="log_id of data to visualize")
    parser.add_argument("ann_file", help="gt file to visualize")
    parser.add_argument("--result", type=str, help="prediction result to visualize")
    parser.add_argument(
        "--thr", type=float, default=0, help="score threshold to filter predictions"
    )
    parser.add_argument(
        "--out-dir",
        default="demo",
...
def main(args):
    ...
    for frame in mmcv.track_iter_progress(log_ann):
        ...
        imgs = [
            mmcv.imread(os.path.join(root_path, "argoverse2", i["image_path"]))
        ...
        renderer.render_camera_views_from_vectors(
            annotation, imgs, extrinsics, intrinsics, 4, frame_dir
        )
        ...
        if args.result:
            ...
            renderer.render_camera_views_from_vectors(
                vectors, imgs, extrinsics, intrinsics, 4, frame_dir
            )
